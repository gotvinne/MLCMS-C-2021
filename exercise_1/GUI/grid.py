...
from objects.table import REACHED_TARGET, Table
...
class Grid(): 
    ...
    def __init__(self,simulation_dict,simulation_steps):
       ...
       # NB! Dimentions above 200 (n > 200) is discuraged due to visualization and slow GUI
        ...
        self.table = Table(self.n,simulation_dict)
    ...
    def update_gui(self):
        ...
        if (not self.pause):
            ...
            if (self.table.done_simulation() and self.simulation_steps==0):
                finished_ped = self.table.get_finisched()
    ...
    def print_results_to_file(self,filepath):
        with open(os.path.join(sys.path[0],filepath),'w+') as file:
            finisched_ped = self.table.get_finisched()
            table = PrettyTable(["Pedestrian position","Speed","Initial distance","Simulation steps"])
            for ped in finisched_ped:
                table.add_row(ped.get_results())
